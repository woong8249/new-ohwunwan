...
export async function getComments(postType, post_id, number, limit) {
  let query = `
  select cccu.* ,l.likeCount
from 
(select
    comment_id,text,ccc.createdAt,updatedAt,userId,nickname,picture,replycount
from 
    (select 
        c.id as comment_id,c.user_id,c.text,c.createdAt,c.updatedAt,cc.replyCount
     from 
        (select * from comments_${postType} where ${postType}_id=? and parent is null) as c
     left join
...
left join 
    (select comment_${postType}_id,count(comment_${postType}_id) as likeCount from likes_comment_${postType} group by comment_${postType}_id) as l
 on l.comment_${postType}_id =cccu.comment_id
`;
...
from
  (select
      c.id as comment_id,text,c.createdAt,updatedAt,u.userId,u.nickname,u.picture
  from
...
left join 
  (select comment_${postType}_id,count(comment_${postType}_id) as likeCount from likes_comment_${postType} group by comment_${postType}_id) as l
on 
  l.comment_${postType}_id =cu.comment_id
